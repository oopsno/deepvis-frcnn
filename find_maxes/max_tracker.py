...
from loaders import load_labels, caffe
from jby_misc import WithTimer
from caffe_misc import shownet, RegionComputer, save_caffe_image
from load_data import parse_roi_xml, load_image
import numpy as np
...
default_layers  = ['conv1', 'conv2', 'conv3', 'conv4', 'conv5', 'fc6', 'fc7', 'cls_prob']
...
def load_file_list(filelist):
    """
    loads image filename, roi & label
    """
    image_filenames = []
    image_xmls = []
    image_labels = []
    with open(filelist, 'r') as ff:
        for line in ff.readlines():
            ...
            image_xmls.append(fields[1])
            image_labels.append(int(fields[2]))
    return image_filenames, image_xmls, image_labels
